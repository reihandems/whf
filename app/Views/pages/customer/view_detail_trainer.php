    ...
    <div class="grid grid-cols-12 gap-8">
        ...
        <div class="col-span-12 md:col-span-6">
            ...
            <div class="flex flex-row items-center gap-2 mb-4">
                ...
                <p class="text-xs font-bold text-gray-500 uppercase tracking-widest"><?= $t['jumlah_review'] ?> Reviews
                </p>
            </div>
            ...
            <form action="<?= base_url('/user/checkout-trainer') ?>" method="GET" class="bg-base-100 p-6 rounded-2xl border border-base-content/5 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <fieldset class="fieldset">
                        ...
                        <input type="date" name="tanggal_sesi" class="input input-bordered w-full rounded-xl" required
                            min="<?= date('Y-m-d') ?>" />
                    </fieldset>
                    <fieldset class="fieldset">
        ...
        <div class="col-span-12 mt-16">
            ...
            <?php if (empty($reviews)): ?>
                <div class="bg-base-200/50 p-12 rounded-3xl text-center border-2 border-dashed border-base-content/10">
                    <p class="text-gray-400 font-bold italic">Belum ada ulasan untuk trainer ini. Jadilah yang pertama
                        memberikan ulasan!</p>
                </div>
            <?php else: ?>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <?php foreach ($reviews as $r): ?>
                        <div
                            class="bg-white p-8 rounded-3xl border border-base-content/5 shadow-xl shadow-gray-200/50 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/5">
                                ...
                                <div class="flex flex-row gap-4 items-center">
                                    <div class="avatar shadow-lg shadow-gray-200/50">
                                        <div class="w-12 rounded-2xl">
                                            ...
                                            <?php else: ?>
                                                <img
                                                    src="https://ui-avatars.com/api/?name=<?= urlencode($r['nama_lengkap']) ?>&background=random&bold=true" />
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                                            <?= date('d F Y', strtotime($r['created_at'])) ?></p>
                                    </div>
                                </div>
                            ...
                            <p
                                class="text-sm text-gray-500 font-medium leading-relaxed italic border-l-4 border-primary/20 pl-4 py-1">
                                "<?= esc($r['komentar']) ?>"
                            </p>
                        </div>
                    <?php endforeach; ?>
